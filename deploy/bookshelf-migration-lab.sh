...
BOOKSHELF_IMAGE="${BOOKSHELF_IMAGE:-$DEFAULT_BOOKSHELF_HARDCOVER_IMAGE}"
...
BOOKSHELF_HARDCOVER_NATIVE="${BOOKSHELF_HARDCOVER_NATIVE:-false}"
COMPOSE_PROFILES="${COMPOSE_PROFILES:-rreading-glasses}"
RREADING_GLASSES_IMAGE="${RREADING_GLASSES_IMAGE:-blampe/rreading-glasses:hardcover@sha256:3489e722a73c9cbab5b9ba530cf8a60c2280367fb03db1fb649261dfb064b52f}"
...
ensure_bookshelf_image() {
  case "$BUILD_LOCAL_IMAGE" in
    ...
    auto)
      ...
      elif docker image inspect "$DEFAULT_BOOKSHELF_SOFTCOVER_IMAGE" >/dev/null 2>&1; then
...
prepare_lab() {
  ...
  if [ -z "$HARDCOVER_AUTH" ]; then
    echo "HARDCOVER_AUTH is required for the Hardcover migration lab." >&2
    exit 2
  fi
  if [[ "$HARDCOVER_AUTH" != Bearer\ * ]]; then
  ...
  compose_lab up -d rreading-glasses-postgres rreading-glasses bookshelf-ebooks bookshelf-audiobooks
