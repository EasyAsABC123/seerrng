...
DB_PORT="5432" # (optional) The port to connect to, from 1 through 65535. The default is "5432".
...
DB_POOL_SIZE="10" # (optional) The number of connections to maintain in the connection pool, from 1 through 100. Values below 2 are raised to 2. The default is "10".
DB_LOG_QUERIES="false" # (optional) Whether to log the DB queries for debugging. The default is "false".
```
...
Boolean database options accept only `true` or `false` (case-insensitive). TLS material read through a `_FILE` option must resolve to a regular file no larger than 1 MiB; symlinked secret mounts are supported.
